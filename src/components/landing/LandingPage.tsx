      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className={`fixed inset-0 z-[100] p-6 flex flex-col justify-between ${
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-[#F1917D] p-0.5 bg-black shrink-0">
                  <Image
                    src="/icon-192.png"
                    alt="RMSPS Logo"
                    width={36}
                ...
                <div className="flex flex-col">
                  <span className="text-lg font-display font-black tracking-widest uppercase leading-none">
                    RMSPS
                  </span>
            ...
            {/* Navigation Options - Compact, balanced & clean */}
            <div className="flex flex-col gap-1 py-4">
              <a
                href="#experience"
                onClick={() => setMobileMenuOpen(false)}
                className={`flex items-center justify-between py-3 px-3 rounded-xl text-base font-display uppercase font-bold tracking-wider transition-colors ${
                  isLight
                    ? "hover:bg-black/5 hover:text-[#F1917D]"
                    : "hover:bg-white/5 hover:text-[#F1917D]"
                }`}
              >
                <span>Academics</span>
              ...
              <div className="pt-3">
                <Link
                  href="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="w-full flex items-center justify-center gap-2 bg-[#F1917D] text-white py-3 px-5 rounded-xl text-xs font-mono uppercase tracking-widest font-bold shadow-md active:scale-[0.98] transition-all text-center"
                >
                  Portal Login →
                </Link>
              </div>
            ...
            {/* Footer */}
            <div
              className={`text-[10px] font-mono uppercase border-t pt-4 text-center tracking-wider ${
      ...
      <div className="flex overflow-hidden select-none" style={{ transform: "translateZ(0)" }}>
        <div className="flex items-center shrink-0 animate-marquee gap-12 sm:gap-16 pr-12 sm:pr-16">
          {BADGES.map((badge, i) => (
            <span
              key={`b1-${i}`}
              className="text-base sm:text-xl font-display font-black italic tracking-tighter uppercase opacity-80 shrink-0 whitespace-nowrap"
            >
              ✦ {badge}
            </span>
          ))}
