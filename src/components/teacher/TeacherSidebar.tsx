...
function SidebarContent({ pathname, counts, teacherName, teacherAvatar, onClose, onLogout }: SidebarContentProps) {
        ...
        {NAV_ITEMS.map(({ label, href, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              onClick={onClose}
              className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group ${active ? 'bg-veena-blue text-ink' : 'text-mist hover:text-veena-blue hover:bg-veena-blue/5'}`}
            >
        ...
        {open && (
          <>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="lg:hidden fixed left-0 top-0 bottom-0 z-50 w-72 bg-ink border-r border-hairline">
              ...
              <SidebarContent pathname={pathname} counts={counts} teacherName={teacherName} teacherAvatar={teacherAvatar} onClose={() => setOpen(false)} onLogout={handleLogout} />
            </motion.div>
          </>
        )}
      </AnimatePresence>
