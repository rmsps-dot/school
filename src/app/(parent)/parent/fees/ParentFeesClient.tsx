...
import { Wallet, ShieldCheck, Clock, CheckCircle2, AlertCircle, CreditCard, ChevronRight, Loader2 } from 'lucide-react'
...
export default function ParentFeesClient({ children }: { children: ChildInfo[] }) {
  ...
  const totalDue = fees.filter(f => f.status === 'due').reduce((s, f) => s + Number(f.amount) - Number(f.paid_amount), 0)
  const totalPaid = fees.filter(f => f.status === 'paid' || Number(f.paid_amount) > 0).reduce((s, f) => s + Number(f.paid_amount), 0)
  const totalPending = fees.filter(f => f.status === 'upcoming').reduce((s, f) => s + Number(f.amount), 0)
    ...
    <div className="space-y-8">
      ...
      <div className="surface-card border-hairline rounded-2xl p-5 border-veena-blue/30 flex items-start gap-4 shadow-xl">
        ...
        <p className="text-sm text-mist leading-relaxed">
          ...
          Razorpay integration will allow you to pay fees directly from this portal.
          Contact the school office to make payments in the meantime.
        </p>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <span className="text-mist font-mono text-[10px] uppercase tracking-widest">Loading fee records...</span>
        </div>
      ) : (
        <>
          {/* ── Summary cards ── */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              { label: 'Total Paid',     amount: totalPaid,    icon: CheckCircle2, color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
              { label: 'Due Now',        amount: totalDue,     icon: AlertCircle,  color: 'text-red-400',     bg: 'bg-red-500/10 border-red-500/20'     },
              { label: 'Upcoming',       amount: totalPending, icon: Clock,        color: 'text-amber-400',   bg: 'bg-amber-500/10 border-amber-500/20'   },
            ].map(({ label, amount, icon: Icon, color, bg }, i) => (
              <div key={label} className="surface-card border-hairline rounded-[2rem] p-8 text-center shadow-xl transition-transform hover:-translate-y-1 hover:border-gold/30" style={{ animationDelay: `${i * 0.1}s` }}>
                <div className={`w-12 h-12 rounded-full mx-auto flex items-center justify-center mb-4 border shadow-inner ${bg}`}>
                  <Icon className={`w-6 h-6 ${color}`} />
                </div>
                <p className={`font-display text-4xl font-black drop-shadow-md ${color}`}>₹{amount.toLocaleString('en-IN')}</p>
          ...
          <div className="surface-card border-hairline rounded-[2rem] overflow-hidden shadow-2xl">
            <div className="px-8 py-5 bg-surface border-b border-hairline flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-[10px] font-bold text-gold uppercase tracking-widest">Fee Ledger</h2>
              </div>
              Student: <span className="text-xs font-bold text-parchment">{activeChild?.fullName}</span>
            </div>

            <div className="divide-y divide-hairline">
              {fees.length === 0 ? (
                <div className="p-16 text-center">
                  <p className="text-mist font-mono uppercase tracking-widest text-xs">No fee records found for this student.</p>
                </div>
              ) : (
                fees.map((fee, i) => {
                  const statusMap: Record<string, { label: string; cls: string; Icon: React.ElementType }> = {
                    ...
                    due:      { label: 'Due',      cls: 'bg-red-500/10     text-red-400     border-red-500/20',     Icon: AlertCircle  },
                    upcoming: { label: 'Upcoming', cls: 'bg-amber-500/10   text-amber-400   border-amber-500/20',   Icon: Clock        },
                  }
      ...
      <div className="surface-card border-hairline rounded-2xl p-6 flex items-start gap-4 shadow-lg">
        ...
        <div className="text-sm text-mist space-y-2">
          <p><strong className="text-parchment font-bold text-base">School Account Office</strong></p>
