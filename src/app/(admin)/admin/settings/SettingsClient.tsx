...
import { Save, Clock, Loader2, CheckCircle2, MapPin, Trash2, Users } from 'lucide-react'
...
export default function SettingsClient({ initialSettings }: SettingsClientProps) {
  ...
  // ── 2. Student Attendance Window Setting (Renamed from former Teacher Attendance Window) ──
  ...
  const handleSaveStudentAttendanceWindow = () => {
    ...
    startStudentTransition(async () => {
      // Save both student_attendance_window and alias teacher_attendance_window for backward compatibility
  ...
  return (
    <div className="space-y-10">
      ...
      {/* ─────────────────────────────────────────────────────────────
          SECTION 1: TEACHER ATTENDANCE WINDOW & GEOFENCE LOCATION
      ───────────────────────────────────────────────────────────── */}
      <motion.div
        ...
        <div className="p-6 md:p-8 border-b border-hairline bg-gradient-to-r from-coral/10 via-transparent to-transparent">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="font-display text-2xl font-bold text-parchment">Teacher Attendance & Geofence Settings</h2>
                <p className="text-xs md:text-sm text-mist mt-1">
                  Configure daily photo attendance time window, school GPS coordinates, and allowed radius.
                </p>
              </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold text-parchment uppercase tracking-wider">
                1. Teacher Check-in Time Limits
              </h3>
            </div>
      ...
      {/* ─────────────────────────────────────────────────────────────
          SECTION 2: STUDENT ATTENDANCE WINDOW (RENAMED)
      ───────────────────────────────────────────────────────────── */}
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        className="surface-card rounded-3xl border border-hairline overflow-hidden shadow-xl"
      >
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <div className="pt-2">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-parchment border border-hairline bg-white/5 hover:bg-white/10 hover:border-mist transition-all disabled:opacity-50"
            >
